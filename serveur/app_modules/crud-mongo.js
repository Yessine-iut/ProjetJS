...
exports.countPlats = async (name) => {
	...
	try {
		if (name == '') {
			reponse = await db.collection('plats')
				.countDocuments();
		} else {
			let query = {
				"name": { $regex: ".*" + name + ".*", $options: "i" }
...
exports.findPlats = async (page, pagesize, name) => {
	let client = await MongoClient.connect(url, { useNewUrlParser: true });
	let db = client.db(dbName);
	let reponse;
	...
	try {
		...
		if (name == '') {
			plats = await db.collection('plats')
				.find()
				.skip(page * pagesize)
				.limit(pagesize)
				.toArray();

			count = await db.collection('plats').countDocuments();
		} else {
			let query = {
				"name": { $regex: ".*" + name + ".*", $options: "i" }
			}
			...
			count = await db.collection('plats')
				.find(query)
				.count();
		}
		// On renvoie la réponse sous la forme d'une promesse (la fonction est async)
		reponse = {
			succes: true,
			msg: "plats recherchés avec succès",
			data: plats,
			count: count
		}
	} catch (err) {
...
exports.findPlatById = async (id) => {
	...
	try {
		...
		let data = await db.collection("plats").findOne(myquery);

		reponse = {
			succes: true,
			restaurant: data,
			error: null,
			msg: "Details du plat envoyés"
		};
	} catch (err) {
		reponse = {
			succes: false,
...
exports.createPlat = async (formData) => {
	let client = await MongoClient.connect(url, { useNewUrlParser: true });
	let db = client.db(dbName);
	let reponse;
	...
	try {
		...
		let data = await db.collection("plats").insertOne(toInsert);
		reponse = {
			succes: true,
			result: toInsert._id,
			msg: "Ajout réussi " + toInsert._id
...
exports.updatePlat = async (id, formData) => {
	let client = await MongoClient.connect(url, { useNewUrlParser: true });
	let db = client.db(dbName);
	let reponse;
	...
	try {
		...
		let result = await db.collection("plats").updateOne(myquery, newvalues);
...
exports.deletePlat = async function (id, callback) {
	let client = await MongoClient.connect(url, { useNewUrlParser: true });
	let db = client.db(dbName);
	let reponse;
	...
	try {
		...
		let result = await db.collection("plats")
			.deleteOne(myquery);
		reponse = {
			succes: true,
			result: result,
